...
def iplot(pb_obj, use_gl=False):
    ...
    if len(pb_obj.ch_set) == 0:
        return None

    # Iterate through ch_set and create a trace for each channel
    #  - create sorted list of channels
    ch_list = list(pb_obj.ch_set)
    ch_list.sort()
    ...
    fig['layout'].update(height=100 + 100 * len(pb_obj.ch_set), width=800)
